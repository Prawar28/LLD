...
ll querySumLazy(ll index, ll start, ll left, ll right) {
    // lazy update if any pending
    if (lazy[index] != 0) {
        seg[index] += (right - left + 1) * lazy[index] ;
        // propagate the change to further low range nodes if anys
    ...
    ll mid = (right + left)/2;
    if (start <= mid) return querySumLazy(2 * index + 1, start, left, mid);
    else return querySumLazy(2 * index + 2, start, mid + 1, right);
}
...
int main()
{
    ...
    while(q--) {
        ...
        if (qt == 1) {
            cin >> a >> b >> u;
            lazyRangeUpdate(0, 0, n - 1, --a, --b, u);
        } else {
            cin >> a;
            --a;
            // cout << a << endl;
            cout << querySumLazy(0, a, 0, n - 1) << endl;
        }
    }
}
